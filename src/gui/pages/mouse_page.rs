...
use iced::{
   button, slider, Align, Button, Checkbox, Column, Container, Element, Length, Radio, Row, Slider,
   Space, Svg, Text,
};
...
impl MousePage {
   ...
   pub fn view(&mut self) -> Element<MouseMessage> {
      ...
      // ផ្ទាំងខាងឆ្វេង
      let logo = Svg::from_path(format!("{}/assets/images/mouse.svg", ROOT_PATH()))
            ...
            row.push(
               Radio::new(
                  *button,
                  &format!("{:?}", button),
                  Some(*primary_button),
                  MouseMessage::PrimaryButtonChanged,
               )
               .size(15)
               .spacing(10)
               .style(if *primary_button == *button {
                  CustomRadio::Active
               } else {
      ...
      let slider_pointer_speed = Slider::new(
         pointer_speed_state,
         0..=100,
         *pointer_speed,
         MouseMessage::PointerSpeedChanged,
      )
      .width(Length::Units(200))
      .style(CustomSlider::Default);
      let pointer_speed_row = Row::new()
         .spacing(15)
      ...
      let slider_double_click_speed = Slider::new(
         double_click_speed_state,
         0..=100,
         *double_click_speed,
         MouseMessage::DoubleClickSpeedChanged,
      )
      .width(Length::Units(200))
      .style(CustomSlider::Default);
      let double_click_speed_row = Row::new()
         .spacing(15)
         .align_items(Align::Center)
         .push(lb_double_click_speed)
         .push(slider_double_click_speed);

      let chb_reverse = Checkbox::new(
         *reverse_scrolling,
         "Reverse Scrolling Direction",
         MouseMessage::ReverseScrollingToggled,
      )
      .spacing(10)
      .style(CustomCheckbox::Default);
      let reverse_row = Row::new()
         .spacing(15)
      ...
      // ផ្នែកខាងក្រោម
      let btn_restore_default = Button::new(btn_default_state, Text::new("  Defaults  "))
         .on_press(MouseMessage::RestoreDefaultClicked)
         .style(CustomButton::Default);
      let bottom = Row::new()
         .spacing(15)
         .align_items(Align::Center)
         .push(btn_restore_default);
