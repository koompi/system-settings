...
use iced::{
   button, scrollable, Scrollable, Button, Text, Container, Length, Column, Row, Align, Element,
};
use iced_custom_widget::Icon;
use crate::gui::styles::{CustomButton, CustomContainer};
use users::{
   Group, Groups, Users, get_current_gid, group_access_list,
};
...
impl GroupsTab {
   ...
   pub fn view(&mut self) -> Element<GroupsMsg> {
      use GroupsMsg::*;
      let Self {
         ls_grps, selected_grp, scroll_grps, add_state, remove_state, edit_group_page, add_group_page, ..
      } = self;
      ...
      let scrollable_group = ls_grps.iter_mut().enumerate().fold(Scrollable::new(scroll_grps).height(Length::Fill).padding(7).spacing(4).scroller_width(4).scrollbar_width(4), |scrollable, (idx, (group, state))| {
         let btn = Button::new(state, Text::new(group.name().to_str().unwrap_or(""))).width(Length::Fill)
