...
use crate::gui::styles::{CustomTextInput, CustomCheckbox, CustomContainer, CustomButton, FOREGROUND};
...
impl EditGroupPage {
   ...
   pub fn view(&mut self) -> Element<EditGroupMsg> {
      ...
      let txt_grp_name = TextInput::new(group_name_state, "Group name", &group_name_val, GroupNameChanged).padding(7).width(Length::Fill)
         .style(CustomTextInput::Default).on_submit(GroupNameSubmitted(group_name_val.clone()));
      let con_grp_name: Element<_> = if is_admin {
         txt_grp_name.into()
      } else {
         Stack::new()
         .push(txt_grp_name, None)
         .push(Container::new(Row::new()).width(Length::Units(120)).height(Length::Units(25)).style(CustomContainer::Transparent(FOREGROUND)), None).into()
      };
