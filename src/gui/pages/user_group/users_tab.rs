...
impl UsersTab {
   ...
   pub fn view(&mut self) -> Element<UsersMsg> {
      ...
      let scrollable_users = ls_users.iter_mut().enumerate().fold(Scrollable::new(scroll_users).height(Length::Fill).padding(7).spacing(4).scroller_width(4).scrollbar_width(4), |scrollable, (idx, (user, state))| {
         ...
         //    profile_path = ;
         // }
         let content = Row::new().spacing(10).align_items(Align::Center)
            .push(Container::new(Image::new(PathBuf::from("/usr/share/sddm/faces/root.face.icon"))).width(Length::Units(30)).height(Length::Units(30)).style(CustomContainer::Header))
            .push(Text::new(user.name().to_str().unwrap_or("")));
         let btn = Button::new(state, content).width(Length::Fill).on_press(SelectedUsr(idx))
            .style(
