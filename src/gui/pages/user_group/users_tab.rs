...
use {
   ...
   change_groups_page::{ChangeGroupsPage, ChangeGroupsMsg},
};
use std::cell::{RefCell, RefMut};
use libkoompi::system_settings::users_groups::{User, UsersGroupsManager, AccountType};
use iced::{
   button, scrollable, Scrollable, Button, Text, Container, Length, Column, Row, Align, Element, Image, Space,
};
...
impl UsersTab {
                     ...
                     match usr_info_msg {
                        ...
                        AllowUsrAdminToggled(is_checked) => if curr_is_admin {
                           match usrgrp_mn.borrow_mut().change_user_type(usr.username(), if is_checked {AccountType::Admin} else {AccountType::default()}) {
                              Ok(newusr) => if let Some(newusr) = newusr {
   ...
   pub fn view(&mut self) -> Element<UsersMsg> {
      ...
      let scrollable_users = ls_users.iter_mut().enumerate().fold(Scrollable::new(scroll_users).height(Length::Fill).spacing(4).scroller_width(4).scrollbar_width(4), |scrollable, (idx, (user, state))| {
         ...
         let row_btn = Row::new().width(Length::Fill)
         ...
         scrollable.push::<Element<_>>(match idx {
            0 => {
               ...
               .push(Container::new(
                  Container::new(Text::new("Current User").size(11)).width(Length::Fill).padding(3).style(CustomContainer::Header)
            ...
            1 => {
               Column::new().spacing(2)
               .push(Container::new(
                  Container::new(Text::new("Other Users").size(11)).width(Length::Fill).padding(3).style(CustomContainer::Header)
               ).width(Length::Fill).padding(2))
               .push(row_btn).into()
            },
            _ => row_btn.into()
      ...
      let right_sec = match content {
         ...
         Empty => Container::new(
            Text::new("There is no users available")
