...
impl UsersTab {
               ...
               if let Some(idx) = self.selected_user {
                  if let Some((user, _)) = ls_users.get_mut(idx) {
                     ...
                     match usr_info_msg {
                        ...
                        ChangeInfoClicked => {
                           self.content = ChangeInfo(ChangeInfoPage::new(
                              user,
                              usrgrp_mn.borrow().all_groups().iter().find(|grp| grp.gid() == user.gid()).map(|grp| grp.name()),
                              usrgrp_mn.borrow().login_shells().to_vec(),
                           ))
                        }
   ...
   pub fn view(&mut self) -> Element<UsersMsg> {
      ...
      let scrollable_users = ls_users
         ...
         .fold(Scrollable::new(scroll_users).height(Length::Fill).padding(7).spacing(4).scroller_width(4).scrollbar_width(4), |scrollable, (idx, (user, state))| {
            ...
            let row = Row::new()
               ...
               .push(
                  Container::new(Image::new(user.home_dir().join(".face").to_path_buf()).width(Length::Units(30)).height(Length::Units(30)))
      ...
      Container::new(Row::new().width(Length::Fill).spacing(10).push(users_pane).push(right_sec)).width(Length::Fill).height(Length::Fill).into()
   }
}
