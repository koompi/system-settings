...
use users::{get_user_by_uid, uid_t, User};
...
impl ChangePwdPage {
    ...
    pub fn view(&mut self) -> Element<ChangePwdMsg> {
        ...
        let lb_sec = Column::new().spacing(20).align_items(Align::End).push(lb_old_pwd).push(lb_new_pwd).push(lb_verify_pwd).push(lb_pwd_hint);
        ...
        let input_sec = Column::new().spacing(5).push(txt_old_pwd).push(Row::new().spacing(10).push(txt_new_pwd).push(btn_show_pwd)).push(txt_verify_pwd).push(txt_pwd_hint);
        ...
        Container::new(
            Column::new()
                ...
                .push(Row::new().spacing(10).align_items(Align::Center).push(lb_sec).push(input_sec))
                .push(Space::with_height(Length::Fill))
