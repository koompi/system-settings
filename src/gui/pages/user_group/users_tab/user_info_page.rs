...
use iced::{button, Align, Button, Checkbox, Column, Container, Element, Image, Length, Row, Space, Text};
   ...
   pub fn view(&mut self) -> Element<UserInfoMsg> {
      ...
      let chb_allow_usr_admin: Element<_> = if self.allow_usr_admin && !self.is_curr_usr {
         ...
         Stack::new()
            ...
            .push(Container::new(Row::new()).width(Length::Units(200)).height(Length::Units(25)).style(CustomContainer::Transparent(FOREGROUND)), None)
            .into()
      };
      ...
      Container::new(
         Column::new()
            .padding(20)
            .push(Row::new().spacing(15).align_items(Align::Center).push(profile).push(txt_username).push(Space::with_width(Length::Fill)).push(btn_change_pwd))
            .push(Row::new().align_items(Align::Center).push(chb_allow_usr_admin).push(Space::with_width(Length::Fill)).push(btn_change_info)),
      )
