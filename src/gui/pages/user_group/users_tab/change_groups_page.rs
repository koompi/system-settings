use iced::{
   button, text_input, scrollable, Scrollable, TextInput, Text, Container, Length, Column, Row, Align, Checkbox, Element, Space,
};
use libkoompi::system_settings::users_groups::{Group, User};
use crate::gui::styles::{CustomCheckbox, CustomContainer, CustomButton, CustomTextInput};
...
impl ChangeGroupsPage {
   ...
   pub fn view(&mut self) -> Element<ChangeGroupsMsg> {
      use ChangeGroupsMsg::*;
      let Self {
         grpname_state, grpname, filtered_ls_grps, scroll_grps, btn_cancel_state, btn_ok_state, ..
      } = self;
      ...
      let scrollable_grps = filtered_ls_grps.iter_mut().enumerate().fold(
         Scrollable::new(scroll_grps).height(Length::Fill).padding(7).spacing(4).scroller_width(4).scrollbar_width(4), |scrollable, (idx, (is_checked, grp))| {
