...
use crate::gui::styles::{CustomButton, CustomContainer, CustomSelect, CustomTextInput, FOREGROUND};
use iced::{button, pick_list, text_input, Align, Column, Container, Element, Length, PickList, Row, Space, Text, TextInput};
use iced_custom_widget::{number_input, Icons, NumberInput, Stack};
use libkoompi::{
   helpers::to_account_name,
   system_settings::users_groups::{User, MAX_UID, MIN_UID},
};
...
impl ChangeInfoPage {
   ...
   pub fn view(&mut self) -> Element<ChangeInfoMsg> {
      ...
      let mut btn_browse_home = icon_btn(btn_browse_home_dir, Icons::Info, "Browse", None).style(CustomButton::Default);
      if !self.is_curr_usr {
      ...
      let (sec_uid, sec_usrname, sec_home_dir): (Element<_>, Element<_>, Element<_>) = if !self.is_curr_usr {
         ...
         let txt_uid = Stack::new()
            .push(txt_user_id, None)
         ...
         let txt_usrname = Stack::new()
            .push(txt_username, None)
            .push(Container::new(Row::new()).width(Length::Fill).height(Length::Units(27)).style(CustomContainer::Transparent(FOREGROUND)), None);
         let sec_home_dir = Stack::new()
            .push(sec_home_dir, None)
            .push(Container::new(Row::new()).width(Length::Fill).height(Length::Units(27)).style(CustomContainer::Transparent(FOREGROUND)), None);
         (txt_uid.into(), txt_usrname.into(), sec_home_dir.into())
      };

      let info_sec = Column::new().spacing(7).push(sec_uid).push(txt_fullname).push(sec_usrname).push(txt_group_name).push(pl_login_shell).push(sec_home_dir);
      ...
      Container::new(
         Column::new()
            ...
            .push(Row::new().padding(10).spacing(10).width(Length::Units(400)).align_items(Align::Center).push(lb_sec).push(info_sec))
