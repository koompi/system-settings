...
impl AddUserPage {
    ...
    pub fn view(&mut self) -> Element<AddUserMsg> {
        ...
        let lb_sec = Column::new().spacing(20).align_items(Align::End).push(lb_fullname).push(lb_username).push(lb_acc_type).push(lb_pwd).push(lb_verify_pwd).push(lb_pwd_hint);
        ...
        Container::new(
            Column::new()
                ...
                .push(Row::new().spacing(10).align_items(Align::Center).push(lb_sec).push(info_sec))
                .push(Space::with_height(Length::Fill))
