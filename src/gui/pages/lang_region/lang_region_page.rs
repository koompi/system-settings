...
impl LangRegionPage {
    ...
    pub fn update(&mut self, msg: LangRegionMessage) {
        ...
        match msg {
            ...
            BtnAddAppClicked => {
                self.apps_tab.app_list.push(('\u{f120}', String::from("Terminal"), pick_list::State::default(), String::from("terminal"), button::State::new()));
                self.is_changed = true;
            }
            BtnRemoveAppClicked => {
    ...
    pub fn view(&mut self) -> Element<LangRegionMessage> {
        ...
        let tabview = match self.current_tab_idx {
            0 => {
                ...
                let btn_add = Button::new(add_state, Icon::new('\u{f067}').size(23)).padding(2).style(CustomButton::Text).on_press(LangRegionMessage::BtnAddClicked);
                ...
                let lang_group = prefered_langs
                    .iter_mut()
                    .enumerate()
                    .fold(Scrollable::new(prefered_lang_scroll).height(Length::Fill).padding(7).spacing(4), |scroll, (idx, (prefered_lang, state))| {
                ...
                let left_pane = Container::new(
                    Row::new()
                        ...
                        .push(Container::new(Column::new().push(lang_group).push(btn_group)).width(Length::Fill).height(Length::Fill).style(CustomContainer::ForegroundWhite))
                    ...
                    let scrollable_prefered_lang = filtered_add_langs
                        ...
                        .fold(Scrollable::new(content_scroll).height(Length::Fill).padding(7).spacing(4).scroller_width(4).scrollbar_width(4), |scrollable, (prefered_lang, state)| {
                            let btn = Button::new(state, Text::new(format!("{}", prefered_lang)))
                                .width(Length::Fill)
                                .on_press(LangRegionMessage::AddLangMsg(AddLangMessage::AddLangChanged(prefered_lang.clone())))
                                ...
                                Column::new()
                                    ...
                                    .push(
                                        TextInput::new(search_prefered_lang_state, "Type language name that's you wish to add", search_prefered_lang_val.as_str(), move |val| {
                                            LangRegionMessage::AddLangMsg(AddLangMessage::SearchPreferedLang(val))
                                        })
                                        .padding(10)
            ...
            1 => {
                ...
                let apps_group = app_list
                    .iter_mut()
                    .enumerate()
                    .fold(Scrollable::new(scroll).height(Length::Fill).width(Length::Fill).padding(7).spacing(4), |scroll, (idx, (icon, title, pl_state, selected_lang, state))| {
                        let content = Row::new()
                            .spacing(7)
                            .padding(4)
                            .align_items(Align::Center)
