use super::super::styles::{CustomButton, CustomCheckbox, CustomContainer, CustomSelect};
...
use iced_custom_widget::{number_input, Icon, IconBrand, NumberInput};
    ...
    pub fn view(&mut self) -> Element<PrivacyMessage> {
        ...
        let tabbar = tabbar_state.iter_mut().enumerate().fold(Row::new().spacing(2).align_items(Align::Center), |tabbar, (idx, (title, state))| {
            let btn = Button::new(state, Text::new(*title))
                .padding(5)
                .on_press(PrivacyMessage::TabChanged(idx))
                .style(if *current_tab_idx == idx { CustomButton::SelectedTab } else { CustomButton::Tab });
            tabbar.push(btn)
        });
        ...
        let tabview = match self.current_tab_idx {
            0 => {
                ...
                let chb_show_msg_screen_lock = Checkbox::new(*show_msg_screen_lock, "Show a message when the screen is locked", PrivacyMessage::ShowMsgScreenLockToggled)
                    .spacing(10)
                ...
                let chb_req_admin = Checkbox::new(*req_admin_pw_sys_pref, "Require an administrator password to access system preferences", PrivacyMessage::ReqAdminPWToggled)
                    .spacing(10)
                    .style(CustomCheckbox::Default);
            ...
            1 => {
                ...
                // ផ្ទាំងខាងឆ្វេង
                let left_tab_col = privacy_tabs
                    .iter_mut()
                    .enumerate()
                    .fold(Scrollable::new(left_pane_scroll).height(Length::Fill).padding(7).spacing(4), |scrollable, (idx, (icon, title, state))| {
                        let btn = Button::new(state, Row::new().spacing(7).align_items(Align::Center).push(Icon::new(*icon).size(30)).push(Text::new(*title)))
                            .width(Length::Fill)
                            .on_press(PrivacyMessage::PrivacyTabSelected(idx))
                ...
                let apps_list_view = ls_apps
                    .iter_mut()
                    .enumerate()
                    .fold(Scrollable::new(right_pane_scroll).width(Length::Fill).height(Length::Fill).padding(7).spacing(4), |col, (idx, (is_checked, icon, title))| {
                        let row = Row::new()
                            .width(Length::Fill)
            ...
            2 => {
                ...
                let btn_turn_firewall = Button::new(turn_firewall_state, Text::new(format!("  Turn {} Firewall  ", firewall_state(!(*is_turn_on_firewall)))))
                    .on_press(PrivacyMessage::TurnFirewallClicked)
                ...
                let chb_auto_allow_built_in_software = Checkbox::new(*auto_allow_built_in_software, "Automatically allow built-in software to recieve incoming connections", PrivacyMessage::AutoAllowBuiltInToggled)
                    .spacing(10)
                    .style(CustomCheckbox::Default);
                let chb_auto_allow_down_signed_software = Checkbox::new(
                    *auto_allow_down_signed_software,
                    "Automatically allow downloaded signed software to recieve incoming connections",
                    PrivacyMessage::AutoAllowDownSignedToggled,
                )
                .spacing(10)
                .style(CustomCheckbox::Default);
        ...
        Self {
            ...
            privacy_tab_map: vec![
                (
                    ...
                    vec![(true, '\u{f289}', "Weather"), (false, '\u{f268}', "Google Chrome"), (true, '\u{f392}', "Discord")],
                ),
                ("Allow the apps below to access your contacts.", None, vec![(true, '\u{f41a}', "Terminal")]),
                ("Allow the apps below to access your calendars.", None, vec![(true, '\u{f41a}', "Terminal")]),
                ("Allow the apps below to access your photos.", None, vec![(true, '\u{f41a}', "Terminal")]),
                ("Allow the apps below to access your camera.", None, vec![(true, '\u{f268}', "Google Chrome")]),
                ("Allow the apps below to access your microphone.", None, vec![(true, '\u{f268}', "Google Chrome"), (true, '\u{f2c6}', "Telegram")]),
