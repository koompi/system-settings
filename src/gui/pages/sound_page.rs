...
impl SoundPage {
   ...
   pub fn update(&mut self, msg: SoundMessage) {
      match msg {
         ...
         SoundMessage::ToggleMute(val) => {
            self.mute = val;
            self.output_volumn_value = if val {
               0
            } else {
               50
            };
   ...
   pub fn view(&mut self) -> Element<SoundMessage> {
      ...
      let output_volumn_row = Row::new().spacing(10).align_items(Align::Center)
         ...
         .push(ic_volumn_up)
         .push(chk_mute);
      let bottom_col = Column::new().spacing(10).align_items(Align::Center)
...
impl SoundEffectDevice {
   const ALL: [SoundEffectDevice; 2] = [SoundEffectDevice::OutputDevice, SoundEffectDevice::Speaker];
}
