...
use iced::{button, pick_list, scrollable, Align, Button, Checkbox, Column, Container, Element, Length, PickList, Row, Scrollable, Space, Svg, Text};
...
impl DesktopPage {
    pub fn new() -> Self {
        Self {
            tabbar_state: vec![("  Desktop  ", button::State::new()), ("  Screen Saver  ", button::State::new())],
            current_tab_idx: 0,
        ...
        match msg {
            ...
            WallpaperModeChanged(val) => {
                self.desktop_tab
                    ...
                    .get_mut(self.desktop_tab.selected_category_folder)
                    .unwrap()
                    .get_mut(self.desktop_tab.selected_wallpaper)
                    .unwrap()
                    .kind = val;
            }
    ...
    pub fn view(&mut self) -> Element<DesktopMessage> {
        ...
        let tabbar = tabbar_state.iter_mut().enumerate().fold(Row::new().spacing(2).align_items(Align::Center), |tabbar, (idx, (title, state))| {
            let btn = Button::new(state, Text::new(*title))
                .padding(5)
                .on_press(DesktopMessage::TabChanged(idx))
        ...
        let tabview = match self.current_tab_idx {
            0 => {
                ...
                // ផ្ទាំងខាងឆ្វេង
                let (default_category_col, photos_category_col, folders_category_col) = categories_folder.iter_mut().enumerate().fold(
                    (Column::new().padding(7).spacing(4), Column::new().padding(7).spacing(4), Column::new().padding(7).spacing(4)),
                    |(default_col, photos_col, folders_col), (idx, (path, title, state, kind))| {
                        let btn = Button::new(
                            state,
                            Row::new()
                                .spacing(7)
                                .align_items(Align::Center)
                                .push(Svg::from_path(format!("{}/assets/images/{}.svg", ROOT_PATH(), *path)).width(Length::Units(20)).height(Length::Units(20)))
                                .push(Text::new(*title)),
                        )
                        .width(Length::Fill)
                        .on_press(DesktopMessage::CategoryFolderSelected(idx))
                ...
                let category_folder_sec = Container::new(
                    Scrollable::new(category_folder_scroll)
                        .padding(5)
                ...
                // ផ្ទាំងខាងស្ដាំ
                let (current_wallpaper_row, wallpaper_grid_view) = categories_wallpaper_map.get_mut(*selected_category_folder).unwrap().iter_mut().enumerate().fold(
                    (Row::new().spacing(20).align_items(Align::Center), Grid::new().column_width(127).padding(10).spacing(10)),
                    |(row, grid), (idx, wallpaper)| {
                        let wallpaper_svg = Svg::from_path(format!("{}/assets/images/wallpaper/{}.svg", ROOT_PATH(), wallpaper.path)).width(Length::Fill).height(Length::Units(55));

                        (
                            if *selected_wallpaper == idx {
                                row.push(Svg::from_path(format!("{}/assets/images/wallpaper/{}.svg", ROOT_PATH(), wallpaper.path)).width(Length::Units(150)).height(Length::Units(100)))
                                    .push(
                                        Column::new()
                                            .spacing(10)
                            ...
                            grid.push(
                                Button::new(&mut wallpaper.btn_state, wallpaper_svg)
                                    .on_press(DesktopMessage::WallpaperSelected(idx))
                                    .style(if *selected_wallpaper == idx { CustomButton::Selected } else { CustomButton::Text }),
                            ),
                        )
                    },
                );
                let wallpaper_sec = Container::new(Scrollable::new(wallpaper_scroll).width(Length::Fill).push(wallpaper_grid_view))
                ...
                let bottom_sec = Container::new(
                    Row::new().spacing(15).push(Space::with_width(Length::FillPortion(3))).push(
                        Column::new()
                            .spacing(5)
                            .width(Length::FillPortion(7))
                            .push(Row::new().spacing(10).align_items(Align::Center).push(chb_change_wall).push(pl_change_wall_dur))
                ...
                Container::new(
                    Column::new()
                        ...
                        .push(Row::new().spacing(15).height(Length::Fill).push(category_folder_sec).push(wallpaper_sec))
                        .push(bottom_sec),
                )
                .width(Length::Fill)
                .height(Length::Fill)
            ...
            1 => {
                ...
                // ផ្ទាំងខាងឆ្វេង
                let (main_view, screensaver_grid_view) = screen_saver_wallpapers
                    .iter_mut()
                    .enumerate()
                    .fold((Column::new().padding(20), Grid::new().column_width(127).padding(10).spacing(10)), |(col, grid), (idx, wallpaper)| {
                        let wallpaper_svg = Svg::from_path(format!("{}/assets/images/screensaver/{}.svg", ROOT_PATH(), wallpaper.path)).width(Length::Fill).height(Length::Units(55));
                        let wallpaper_title = Text::new(wallpaper.name);
                        let wallpaper_con = Container::new(Column::new().spacing(5).align_items(Align::Center).push(wallpaper_svg).push(wallpaper_title));

                        (
                            if *selected_wallpaper == idx {
                                col.push(Svg::from_path(format!("{}/assets/images/screensaver/{}.svg", ROOT_PATH(), wallpaper.path)).width(Length::Units(300)).height(Length::Units(210)))
                            } else {
                                col
                            },
                            grid.push(
                                Button::new(&mut wallpaper.btn_state, wallpaper_con)
                                    .on_press(DesktopMessage::ScreenSaverSelected(idx))
                                    .style(if *selected_wallpaper == idx { CustomButton::Selected } else { CustomButton::Text }),
                            ),
                        )
                    });
                let screen_saver_sec = Container::new(Scrollable::new(wallpaper_scroll).width(Length::Fill).push(screensaver_grid_view))
                ...
                let bottom_sec = Container::new(
                    Row::new()
                        .spacing(15)
                        .push(Row::new().spacing(10).align_items(Align::Center).width(Length::FillPortion(4)).push(lb_start_after).push(pl_start_after_dur))
                        .push(Column::new().spacing(5).width(Length::FillPortion(6)).push(chb_show_clock).push(chb_random_screen_saver)),
                );
                ...
                Container::new(
                    Column::new()
                        ...
                        .spacing(10)
                        .align_items(Align::Start)
                        .push(Row::new().spacing(15).width(Length::Fill).height(Length::Fill).push(screen_saver_sec).push(main_view_sec))
                        .push(bottom_sec),
                )
                .width(Length::Fill)
                .height(Length::Fill)
...
impl WallpaperType {
    const ALL: [WallpaperType; 3] = [WallpaperType::Dyn, WallpaperType::Light, WallpaperType::Dark];
}
...
impl ChangeDuration {
    const ALL: [ChangeDuration; 5] = [ChangeDuration::_15m, ChangeDuration::_30m, ChangeDuration::_1h, ChangeDuration::_2h, ChangeDuration::Everyday];
}
