use iced::{
   button, text_input, scrollable, TextInput, Text, Button, Scrollable, Length, Row, Column, Container, Element, Space
};
...
impl AddInputSrcSec {
   const ls_inp_srcs: [&'static str; 10] = ["English - QWERTY", "English - QWERTZ", "English - AZERTY", "English - QZERTY", "English - QUERTY", "English - AWERTY", "Khmer - QWERTY", "Chinese - QWERTY", "Japenese - QWERTY", "French - QWERTY"];
   ...
   pub fn view(&mut self) -> Element<AddInputSrcMessage> {
      ...
      let scrollable_inp_src = filtered_ls_inp_srcs.iter_mut().fold(Scrollable::new(scroll_inp_src).height(Length::Fill).padding(7).spacing(4).scroller_width(4).scrollbar_width(4), |scrollable, (inp_src, state)| {
         let btn = Button::new(state, Text::new(inp_src.as_str())).width(Length::Fill).on_press(AddInputSrcMessage::InpSrcChanged(inp_src.clone())).style(
            if let Some(selected) = selected_inp_src {
               if selected == inp_src {CustomButton::Selected}
      ...
      let inp_src_pane = Container::new(
         Column::new()
         .push(
            Container::new(Text::new("Add Input Source")).width(Length::Fill).padding(7).style(CustomContainer::Header),
         )
         .push(scrollable_inp_src),
