    ...
    pub fn view(&mut self) -> Element<KeyboardMessage> {
        ...
        let tabview = match self.current_tab_idx {
            0 => {
                ...
                let chk_adjust_brightness = Checkbox::new(*adjust_brightness_low_light, "Adjust keyboard brightness in low light", KeyboardMessage::AdjustBrightnessToggled)
                    .spacing(10)
            ...
            1 => {
                ...
                let left_tab_col = shortcuts_tab.iter_mut().enumerate().fold(Scrollable::new(left_pane_scroll).height(Length::Fill).padding(7).spacing(4), |col, (idx, (icon, title, state))| {
                    col.push(
                        icon_btn(state, *icon, title, Some(23))
                            .width(Length::Fill)
                            .on_press(KeyboardMessage::LeftTabSelected(idx))
                ...
                // ផ្ទាំងខាងស្ដាំ
                let right_pane_col = shortcuts_tab_map
                    .get_mut(*left_pane_selected)
                    .unwrap()
                    .iter_mut()
                    .enumerate()
                    .fold(Scrollable::new(right_pane_scroll).height(Length::Fill).padding(7).spacing(4), |col, (idx, (is_checked, title, shortcut))| {
                        let row = Row::new()
                            .align_items(Align::Center)
                            .padding(4)
                ...
                // ផ្នែកខាងក្រោម
                let chb_keyboard_nav = Checkbox::new(*use_keyboard_nav, "Use keyboard navigations to move focus between controls", KeyboardMessage::KeyNavToggled)
                    .spacing(10)
                    .style(CustomCheckbox::Default);
            ...
            2 => {
                ...
                let right_pane: Element<_> = if *is_config {
                    ...
                    Container::new(Scrollable::new(right_pane_scroll).push(keyboard_image_con))
                ...
                let chb_auto_switch = Checkbox::new(*auto_switch, "Automatically switch to a document's input source", KeyboardMessage::AutoSwitchToggled)
                    .spacing(10)
                    .style(CustomCheckbox::Default);
