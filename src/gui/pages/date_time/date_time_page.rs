...
impl DateTimePage {
    pub fn new() -> Self {
        let tabs = vec![("Date & Time", button::State::new()), ("Time Zone", button::State::new())];
            ...
            Ok(dt_mn) => {
                match LocaleManager::new() {
                    Ok(locale_mn) => {
                        ...
                        let selected_con = dt_mn.list_timezones().iter().find_map(|(key, val)| {
                            if val.contains(&selected_tz.split('/').collect::<Vec<&str>>().last().unwrap().to_string()) {
                                Some(key.to_string())
                            } else {
                                None
                            }
                ...
                match self.current_tab_idx {
                    0 => {
                        ...
                        match DateTime::parse_from_str(
                            datetime.as_str(),
                            format!("{} {} {}", self.locale_manager.time_details().d_fmt.as_str(), self.locale_manager.time_details().t_fmt.as_str(), TZ_FMT).as_str(),
                        ) {
                            Ok(now) => {
    ...
    fn filter_tz(&mut self) {
        self.timezone_tab.filtered_ls_tz = self.timezone_tab.ls_tz.iter().filter(|tz| tz.0.to_lowercase().contains(&self.timezone_tab.search_val.to_lowercase())).cloned().collect();
    }
        ...
        let tabview = match self.current_tab_idx {
            0 => {
                ...
                let txt_date: Element<_> = if *datetime_manager.ntp() {
                    ...
                    TextInput::new(txt_date_state, "", if *is_date_change { temp_date_val } else { &date }, DateTimeMessage::TxtDateChanged)
                        .padding(7)
                ...
                let txt_time: Element<_> = if *datetime_manager.ntp() {
                    ...
                    TextInput::new(txt_time_state, "", if *is_time_change { temp_time_val } else { &time }, DateTimeMessage::TxtTimeChanged)
                        .padding(7)
                        .width(Length::Units(127))
                        .style(CustomTextInput::Default)
                        .into()
                ...
                let calendar_sec = Container::new(Column::new().spacing(20).align_items(Align::Center).push(calendar_con).push(txt_date));

                let canvas_clock = Canvas::new(clock).width(Length::Units(127)).height(Length::Units(127));
                let time_sec = Container::new(Column::new().spacing(20).align_items(Align::Center).push(canvas_clock).push(txt_time));
                let datetime_sec = Container::new(Row::new().spacing(75).align_items(Align::Center).push(calendar_sec).push(time_sec))
                    .width(Length::Fill)
                    .height(Length::Fill)
                    .center_x()
                    .center_y();
                ...
                Container::new(Column::new().spacing(20).push(chb_auto_datetime).push(datetime_sec).push(txt_format_hint)).width(Length::Fill).height(Length::Fill)
            }
            1 => {
                let TimeZoneTab {
                    search_state,
                ...
                let chb_auto_tz = Checkbox::new(*datetime_manager.ntp(), "Set time zone automatically using current location", DateTimeMessage::AutoTZToggled)
                    .spacing(10)
                ...
                let scrollable_continent = ls_continents
                    .iter_mut()
                    .fold(Scrollable::new(scroll_continent).height(Length::Fill).padding(7).spacing(4).scroller_width(4).scrollbar_width(4), |scrollable, (con, state)| {
                        let mut btn = Button::new(state, Text::new(con.as_str())).width(Length::Fill).style(if let Some(selected) = selected_continent {
                            if selected == con {
                ...
                let continent_pane = Container::new(Column::new().push(Container::new(Text::new("Continents")).width(Length::Fill).padding(7).style(CustomContainer::Header)).push(scrollable_continent))
                    .height(Length::Fill)
                    .width(Length::Fill)
                    .style(CustomContainer::ForegroundWhite);

                let scrollable_tz = filtered_ls_tz
                    .iter_mut()
                    .fold(Scrollable::new(scroll_tz).height(Length::Fill).padding(7).spacing(4).scroller_width(4).scrollbar_width(4), |scrollable, (tz, state)| {
                        let mut btn = Button::new(state, Text::new(tz.as_str())).width(Length::Fill).style(if let Some(selected_con) = selected_continent {
                            if datetime_manager.timezone() == &format!("{}/{}", selected_con, tz) {
                                CustomButton::Selected
                ...
                let tz_pane = Container::new(Column::new().push(Container::new(Text::new("Time Zones")).width(Length::Fill).padding(7).style(CustomContainer::Header)).push(scrollable_tz))
                    .height(Length::Fill)
                    .width(Length::Fill)
                    .style(CustomContainer::ForegroundWhite);
