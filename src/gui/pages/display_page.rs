...
use iced::{button, pick_list, scrollable, slider, Align, Button, Checkbox, Column, Container, Element, Length, PickList, Radio, Row, Scrollable, Slider, Space, Svg, Text};
...
impl DisplayPage {
    ...
    pub fn view(&mut self) -> Element<DisplayMessage> {
        ...
        for (idx, (name, btn_state)) in tabbar_state.iter_mut().enumerate() {
            let mut btn = Button::new(btn_state, Text::new(name.as_str())).padding(5).on_press(DisplayMessage::TabChanged(idx));
        ...
        let tabview = match self.current_tab_idx {
            0 => {
                ...
                // ផ្ទាំងខាងឆ្វេង
                let logo = Svg::from_path(format!("{}/assets/images/laptop.svg", ROOT_PATH())).width(Length::Units(150)).height(Length::Units(150));
                let left_pane = Container::new(logo).width(Length::FillPortion(4)).center_x();

                // ផ្ទាំងខាងស្ដាំ
                let lb_resolution = Text::new("Resolution:");
                let rd_resolution = Resolution::ALL.iter().fold(Column::new().spacing(10), |col, option| {
                    col.push(
                        Radio::new(*option, &format!("{:?}", option), Some(*resolution), DisplayMessage::ResolutionChanged)
                            .size(15)
                            .spacing(10)
                            .style(if *resolution == *option { CustomRadio::Active } else { CustomRadio::Disactive }),
                    )
                });
                let resolution_row = Row::new().spacing(15).push(lb_resolution).push(rd_resolution);
                ...
                let brightness_row = Row::new().spacing(15).align_items(Align::Center).push(lb_brightness).push(slider_brightness);

                let chb_auto_adjust_brightness = Checkbox::new(*auto_adjust_brightness, "Automatically adjust brightness", DisplayMessage::AutoAdjustBrightnessToggled)
                    .spacing(10)
                ...
                let right_pane = Container::new(
                    Column::new()
                        ...
                        .push(resolution_row)
                        .push(brightness_row)
                        .push(Row::new().push(Space::with_width(Length::Units(75))).push(Column::new().spacing(10).push(chb_auto_adjust_brightness).push(chb_true_tone).push(txt_hint))),
                )
                .width(Length::FillPortion(6))
                .height(Length::Fill);

                Container::new(Row::new().spacing(15).push(left_pane).push(right_pane)).padding(27).width(Length::Fill).height(Length::Fill)
            }
            1 => {
                let Arrangement {
                    display_pos,
                ...
                let display_con = Container::new(
                    Row::new()
                        .padding(15)
                        .push(Container::new(display).width(Length::Fill).height(Length::Fill).center_x().center_y())
                ...
                Container::new(Column::new().spacing(15).push(txt_hint).push(display_con).push(chb_mirror_display)).width(Length::Fill)
            }
            2 => {
                let Color {
                    display_profiles,
                ...
                let left_pane = Container::new(
                    Column::new()
                        ...
                        .push(Container::new(profile_pane).height(Length::Fill).width(Length::Fill).padding(7).style(CustomContainer::ForegroundWhite))
                ...
                let btn_group = Column::new()
                    ...
                    .align_items(Align::End)
                    .push(btn_create)
                    .push(Row::new().spacing(15).align_items(Align::Center).push(Text::new(opened_display.as_str())).push(btn_open))
                    .push(btn_delete);
        ...
        // ផ្នែកខាងក្រោម
        let chb_show_mirror = Checkbox::new(*show_mirror, "Show mirroring options in the menubar when available", DisplayMessage::ShowMirrorToggled)
            .spacing(10)
            .style(CustomCheckbox::Default);
        let bottom_section = Container::new(chb_show_mirror).padding(15);

        // មាតិកា
        let content = Column::new().push(tabbar_section).push(tabview.height(Length::Fill).padding(20).style(CustomContainer::ForegroundGray)).push(bottom_section);
...
impl NightShift {
    pub fn new() -> Self {
        Self { color_temp_val: 27, ..Default::default() }
    }
}
