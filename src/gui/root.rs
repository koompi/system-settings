...
use iced_custom_widget::{Grid, Icon, Icons};
...
impl Application for SystemSetting {
   ...
   fn view(&mut self) -> Element<Self::Message> {
      ...
      if self.selected_pref.is_some() {
         search_bar = search_bar.push(Button::new(&mut self.back_btn_state, Icon::new(Icons::Search).size(20)).on_press(SystemMessage::NavigateBack).padding(7).style(CustomButton::Text));
      }
      search_bar = search_bar.push(search_section);

      let sidebar = if let Some(selected_pref) = &self.selected_pref {
         let (personal_prefs, device_prefs) = self
            .prefs
            .iter_mut()
            .enumerate()
         ...
         Container::new(Scrollable::new(&mut self.sidebar_scroll).padding(15).spacing(20).scroller_width(4).scrollbar_width(4).push(personal_section).push(device_section))
      ...
      Container::new(Column::new().spacing(15).width(Length::Fill).push(search_bar).push(main_sec)).into()
   }
}
